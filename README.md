...
4. Click **"Inspect run"** (in the hero, once a run is active — there is currently no other way to open the inspector; see `docs/specs/debugging-and-observability.md`) to open the Runtime Inspector and drill into correlated Strands hooks, OpenTelemetry spans, state diffs, and redactions. This currently ships as an Overview + Timeline slice; the fuller Execution/State/Context/Errors views described in `docs/specs/debugging-and-observability.md` are tracked as follow-on work, not yet built.
