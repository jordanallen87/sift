...
- **No live model inference path.** `createBedrockModel`/`resolveModelProvider` exist and are unit-tested, but no production code path reaches them: both engines construct their scripted provider unconditionally, so every run — local and deployed — is scripted. The scripted provider is a real Strands `Model` subclass driving the genuine agent loop, tool-calling and structured-output validation, so **the orchestration in every row above is real**; what is absent is a call to Bedrock. See `docs/specs/strands-runtime.md` "What actually ships".
