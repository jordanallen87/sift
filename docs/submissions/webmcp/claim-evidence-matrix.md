...
| E8 | "Sift emits OpenTelemetry spans." | It does now, as of 2026-09-04 — but it never *emitted* them: `@strands-agents/sdk@1.14.0` already creates them on every Graph/Swarm/agent/model/tool call, and Sift simply never registered a `TracerProvider`, so they were discarded. `apps/agent/src/runtime/otel-span-recorder.ts` registers a real `NodeTracerProvider`, calls the SDK's own `setupTracer({ provider })`, and records each span into `runtime_events` with its real `span_id`/`parent_span_id` and a real span-measured `durationMs` (`apps/agent/src/runtime/otel-span-recorder.test.ts` reconstructs the five-level tree of a real car run from the persisted rows alone). `OTEL_EXPORTER_OTLP_ENDPOINT` now genuinely attaches an OTLP exporter; unset, nothing opens a socket. **Still not claimed:** `setupMeter()`/metrics, W3C `traceparent` propagation to AgentCore, and Sift-authored attributes *on* the spans (Sift's correlation ids live on the `runtime_events` row, not inside the span). |
| E9 | "The Runtime Inspector's `traceId` is a distributed-tracing id." | It is Sift-minted — `deps.idGenerator.next('trace')` in `strands-adapter.ts`, `car-purchase-graph.ts`, `home-energy-swarm.ts` — one id per real Graph/Swarm invocation, stamped onto every normalized event by `event-normalizer.ts`. It correlates Sift's own records (activity event → runtime event → state diff, proven by D4) and nothing outside Sift. Since 2026-09-04 a span row *also* carries the real OTel trace id, but under its own name (`attributes['otel.trace_id']`); `runs.trace_id` and `runtime_events.trace_id` remain Sift's id, and nothing propagates a `traceparent` across a process boundary. |
