...
| E8 | "Sift emits OpenTelemetry spans." | It does not. No Sift code calls the Strands SDK's `setupTracer()`/`setupMeter()`, and the SDK is explicit that its telemetry module "is only loaded when the user explicitly imports and calls setupTracer or setupMeter" (`@strands-agents/sdk/dist/src/telemetry/config.d.ts`). No `@opentelemetry` package is a direct dependency of any workspace, no span processor or OTLP exporter exists, and `OTEL_EXPORTER_OTLP_ENDPOINT`/`OTEL_EXPORTER_OTLP_HEADERS` have no effect. `docs/specs/debugging-and-observability.md` ("OpenTelemetry and AgentCore") and `PAX-P20` still require it; that requirement is **unmet**, and `docs/superpowers/plans/2026-08-26-pax-hackathon-build.md` Task 11 now records it unchecked. What *is* real: `RuntimeCorrelation.spanId`/`parentSpanId` exist in the contract but are never populated (`apps/agent/src/runtime/event-normalizer.ts` header: the normalizer populates correlation "only from run/case/session/obligation/agent identifiers actually available to the runtime layer today, never a fabricated span ID"). |
| E9 | "The Runtime Inspector's `traceId` is a distributed-tracing id." | It is Sift-minted — `deps.idGenerator.next('trace')` in `strands-adapter.ts`, `car-purchase-graph.ts`, `home-energy-swarm.ts` — one id per real Graph/Swarm invocation, stamped onto every normalized event by `event-normalizer.ts`. It correlates Sift's own records (activity event → runtime event → state diff, proven by D4) and nothing outside Sift. |
