...
- [x] Configure Strands OpenTelemetry with a Pax SQLite span processor and optional standard OTLP exporter; add case/run/pack ID-version-hash/obligation/extension-origin attributes. **DONE 2026-09-04** (box was corrected to `[ ]` on 2026-09-03 when verification showed it unbuilt). `apps/agent/src/runtime/otel-span-recorder.ts` registers a real `NodeTracerProvider`, calls the SDK's own `setupTracer({ provider })`, and persists the Strands SDK's already-emitted spans into `runtime_events` with real `span_id`/`parent_span_id` and span-measured durations; `OTEL_EXPORTER_OTLP_ENDPOINT` attaches a `BatchSpanProcessor` + `OTLPTraceExporter` and is off unless set. **Partial, and stated as such:** the case/run/obligation/agent correlation is recorded on the `runtime_events` row rather than injected as attributes *on* the span, because `Graph`/`Swarm`/`Agent` `traceAttributes` are fixed at construction inside the orchestration files; `setupMeter()`/metrics and W3C `traceparent` propagation remain unbuilt. See `docs/specs/debugging-and-observability.md` "OpenTelemetry and AgentCore".
