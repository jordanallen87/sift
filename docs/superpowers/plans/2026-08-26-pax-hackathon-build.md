...
- [ ] Configure Strands OpenTelemetry with a Pax SQLite span processor and optional standard OTLP exporter; add case/run/pack ID-version-hash/obligation/extension-origin attributes. **NOT DONE — box corrected from `[x]` to `[ ]` on 2026-09-03 after verification:** no Sift code calls `setupTracer()`/`setupMeter()` (`grep -rn "setupTracer" apps packages` returns only a comment), no `@opentelemetry` package is a direct dependency of any workspace, and no span processor or OTLP exporter exists — so no OTEL span is ever produced and `OTEL_EXPORTER_OTLP_*` has no effect. The rest of Task 11 did ship; correlation is carried by real Strands TypeScript lifecycle hooks plus a Sift-minted `traceId`, and the deferral is recorded in `apps/agent/src/runtime/event-normalizer.ts`'s module header. Unmet requirement tracked in `docs/completion-report.md` "Known limitations" and `docs/submissions/webmcp/claim-evidence-matrix.md` rows E8/E9.
