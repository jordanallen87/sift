...
- The two supported demos are **Choose Our Next Car** and **Home Energy Guardian** — the first polished implementations of the generic workspace, not the boundary of what the architecture supports. Later packs (laptops, apartments, contractors, and similar shopping/selection decisions) are explicitly not precluded by this build even though none ship in it.
- Choose Our Next Car is the WebMCP-first submission demo. Home Energy Guardian is the AWS/Strands-first submission demo. They share one engine but have deliberately different interaction models and video narratives — the two heroes are not redesigned to converge on one shape (`docs/decisions/0004-consumer-workspace-information-architecture.md` through `0006-webmcp-two-way-collaboration-contract.md`).
