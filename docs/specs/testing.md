...
**Status: the change-set §61 generic-workspace journey (Quick Pick/List/Compare/Board round-tripped through both the UI and WebMCP, dynamic custom-field creation and population, Decision Profile, notes, and a developer-view correlation click, in one consolidated flow) is specified but not yet implemented as its own Playwright spec.** The checked-in E2E suite today is `car-purchase-journey.spec.ts`, `home-energy-guardian-journey.spec.ts`, `vehicle-catalog-journey.spec.ts`, `reload-persistence.spec.ts`, `error-recovery.spec.ts`, and `keyboard-accessibility.spec.ts`; none of these exercises the four option views, WebMCP-driven view/comparison configuration, or the Decision Profile end to end. Component-level tests for each of the four views and for the WebMCP presentation tools exist and pass, but a consolidated browser journey proving the whole §61 sequence together — including the above-the-fold assertion (below) and the presentation-versus-decision-mutation boundary (also below) at all four viewports on that specific flow — remains required release work, not completed evidence.
